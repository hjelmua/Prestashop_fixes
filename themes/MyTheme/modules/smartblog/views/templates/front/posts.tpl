...
{block name='page_content'}
		...
		<div id="content" class="block">
			<div itemtype="#" itemscope="" id="sdsblogArticle" class="blog-post">
				<h1 class="h1">{$meta_title|escape:'htmlall':'UTF-8'}</h1>
				<div class="sdsarticleHeader">
					{if $smartshowauthor ==1}<i class="material-icons">&#xE7FF;</i><span itemprop="author">{if $smartshowauthorstyle != 0}{$firstname} {$lastname}{else}{$lastname} {$firstname}{/if}</span>{/if}
					<i class="material-icons">&#xE878;</i><span itemprop="dateCreated">{$created|escape:'htmlall':'UTF-8'}</span>
		...
		<div id="comments_block_tab">
			<ul class="footer_links clearfix">
				{foreach from=$posts_previous item="post"}
        			{if isset($post.id_smart_blog_post)}
						<li>
							<a title="{l s='Previous Post' mod='smartblog'}" href="{$smartbloglink->getSmartBlogPostLink($post.id_smart_blog_post,$post.link_rewrite)|escape:'htmlall':'UTF-8'}"><i class="material-icons">&#xE5CB;</i> {l s='Previous Post' mod='smartblog'}</a>
						</li>
					{/if}
				{/foreach}
				{foreach from=$posts_next item="post"}
					{if isset($post.id_smart_blog_post)}
						<li class="pull-right">
							<a title="{l s='Next Post' mod='smartblog'}" href="{$smartbloglink->getSmartBlogPostLink($post.id_smart_blog_post,$post.link_rewrite)|escape:'htmlall':'UTF-8'}">{l s='Next Post' mod='smartblog'} <i class="material-icons">&#xE5CC;</i></a>
						</li>
					{/if}
				{/foreach}
			</ul>
					...
					<div class="smartblogcomments" id="respond">
						...
						<div id="commentInput">
							<form action="" method="post" id="commentform">
								<table>
									<tbody>
										...
										{else}
											<tr>
												<td>
													<span class="required">*</span> {l s='Name:'  mod='smartblog'}
												</td>
												<td>
        									...
        									<tr>
												<td>
													<span class="required">*</span> {l s='E-mail:' mod='smartblog'} <span class="note">{l s='(Not Published)' mod='smartblog'}</span>
												</td>
												<td>
											...
											<tr>
												<td>&nbsp;&nbsp;&nbsp;{l s='Website:'  mod='smartblog'} <span class="note">{l s='Site url with' mod='smartblog'} http://</span></td>
												<td><input type="text" tabindex="3" value="" name="website" class="form-control grey"></td>
											</tr>
										{/if}	
										<tr>
											<td><span class="required">*</span> {l s='Comment:'  mod='smartblog'}</td>
										...
										{if Configuration::get('smartcaptchaoption') == '1'}
											...
											<tr>
												<td>{l s='Type Code' mod='smartblog'}</td><td><input type="text" tabindex="" value="" name="smartblogcaptcha" class="smartblogcaptcha form-control grey"></td>
											</tr>
										{/if}
									</tbody>
								</table>
								...
								<div class="right">
							        <div class="submit">
										<button type="submit" name="addComment" id="submitComment" class="btn btn-primary">{l s='Submit' mod='smartblog'}</button>
									</div>
								</div>
    						</form>
						</div>
						  ...
						  beforeSend: function() {
							...
							success: function(json) {
								if (json['error']) {
											...
											$('#commentInput').before('<div class="warning">' + '<i class="material-icons">&#xE002;</i>' + json['error']['common'] + '</div>');
